...
describe('AddTask streamMaxConnections preference sync', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    resetBatchIdCounter()
  })

  function findConnectionsInput(wrapper: ReturnType<typeof mountDialog>) {
    return wrapper.findAll('input[type="number"]').find((input) => {
      const val = Number((input.element as HTMLInputElement).value)
      return val > 0 && val <= 256
    })
  }

  it('syncs form.streamMaxConnections from preferences when the dialog opens', async () => {
    const { usePreferenceStore } = await import('@/stores/preference')
    const preferenceStore = usePreferenceStore()
    preferenceStore.$patch({ config: { streamMaxConnections: 32 } })
    ...
    const connectionsInput = findConnectionsInput(wrapper)
    expect(connectionsInput).toBeDefined()
    expect(Number((connectionsInput!.element as HTMLInputElement).value)).toBe(32)
  })

  it('re-syncs streamMaxConnections on subsequent opens after preference changes', async () => {
    const { usePreferenceStore } = await import('@/stores/preference')
    const preferenceStore = usePreferenceStore()
    preferenceStore.$patch({ config: { streamMaxConnections: 64 } })
    ...
    preferenceStore.$patch({ config: { streamMaxConnections: 16 } })
    ...
    const connectionsInput = findConnectionsInput(wrapper)
    expect(connectionsInput).toBeDefined()
    expect(Number((connectionsInput!.element as HTMLInputElement).value)).toBe(16)
  })
