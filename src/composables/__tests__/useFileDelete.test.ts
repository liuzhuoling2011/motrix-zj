...
describe('deleteTaskFiles', () => {
  ...
  it('does not trash a file path currently owned by another task', async () => {
    const staleRecord = makeTask({
      gid: 'old-gid',
      status: 'error',
      files: [
        {
    ...
    mockResolveOpenTarget.mockResolvedValue('/downloads/archive.zip')
    mockCheckPathIsDir.mockResolvedValue(false)

    await deleteTaskFiles(staleRecord, { protectedTasks: [staleRecord, activeOwner] })

    expect(mockTrashFile).not.toHaveBeenCalledWith({ path: '/downloads/archive.zip' })
    expect(mockTrashFile).not.toHaveBeenCalledWith({ path: '/downloads/archive.zip.aria2' })
  })

  it('does not trash a folder that contains files owned by another task', async () => {
    const staleRecord = makeTask({
      gid: 'old-gid',
      status: 'error',
    ...
    mockResolveOpenTarget.mockResolvedValue('/downloads/Bundle')
    mockCheckPathIsDir.mockResolvedValue(true)

    await deleteTaskFiles(staleRecord, { protectedTasks: [staleRecord, activeOwner] })

    expect(mockTrashFile).not.toHaveBeenCalledWith({ path: '/downloads/Bundle' })
    expect(mockTrashFile).not.toHaveBeenCalledWith({ path: '/downloads/Bundle.aria2' })
  })
